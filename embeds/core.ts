...
export function defineEmbeddedTool<Data>(
  payload: EmbeddedToolPayload<Data>,
): EmbeddedToolPayload<Data> {
...
export function assertEmbedContract(contract: EmbeddedToolContract): void {
  ...
  if (contract.scope.endpoint !== `/api/embed/${contract.id}.json`)
    throw new Error(`Embed endpoint must match id: ${contract.id}`);
  if (contract.version < 1 || !Number.isInteger(contract.version))
    throw new Error(`Invalid embed version: ${contract.id}`);
  if (contract.scope.writeScope === "browser-local-only" && !contract.scope.storageKey)
    throw new Error(`Local embed needs storage key: ${contract.id}`);
  if (contract.scope.writeScope === "none" && contract.scope.storageKey)
    throw new Error(`Read-only embed cannot reserve storage: ${contract.id}`);
}
