...
`servers/thinkinglabs-mcp/` is a stdio MCP server (`@modelcontextprotocol/sdk`) exposing fixed JSON views (`thinkinglabs://thoughts`, `thinkinglabs://claims/by-tag/{tag}`, `thinkinglabs://predictions/calibration`, etc.) and tools (`query_view`, `contact.precheck`, `contact.send`, `question.submit`, `subscribe_brain_diff`). The store prefers `dist/index.sqlite` and falls back to validated markdown under `content/`. See [`docs/agents/mcp-server.md`](./docs/agents/mcp-server.md).
