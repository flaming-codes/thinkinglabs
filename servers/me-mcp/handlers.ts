...
export function handleQueryView(ctx: HandlerContext, args: QueryViewArgs): ToolTextResult {
  try {
    ...
    return jsonToolResult(
      { view: args.view, source: "error", count: 0, items: [], reason: errorMessage(error) },
      true,
    );
  }
}
...
export function handleContactPrecheck(
  ctx: HandlerContext,
  args: ContactPrecheckInput,
): ToolTextResult {
  try {
    ...
    return jsonToolResult(
      { verdict: "decline", reason: `precheck failed: ${errorMessage(error)}`, matched: null },
      true,
    );
  }
}
...
export function handleContactSend(ctx: HandlerContext, args: ContactSendInput): ToolTextResult {
  try {
    const contact = loadContact(ctx.repoRoot);
    const precheck = precheckContact(contact, {
      intent: args.intent,
    ...
    return jsonToolResult(
      { accepted: false, sent: false, reason: `send failed: ${errorMessage(error)}` },
      true,
    );
  }
}
...
export function handleSubscribeBrainDiff(
  ctx: HandlerContext,
  args: SubscribeBrainDiffInput,
): ToolTextResult {
  const feeds = {
  ...
  try {
    const recent = applyGenericGate(
      buildEntries(walkCommits({ since: args.since, cwd: ctx.repoRoot })),
    );
    return jsonToolResult({ subscribed: true, feeds, since: args.since, recent });
  } catch (error) {
    return jsonToolResult(
      { subscribed: false, feeds, since: args.since, reason: errorMessage(error) },
      true,
    );
  }
}
...
export function handleQuestionSubmit(
  ctx: HandlerContext,
  args: QuestionSubmitInput,
): ToolTextResult {
  if (!SAFE_SLUG.test(args.questionSlug)) {
    return jsonToolResult(
      {
        accepted: false,
        reason: "questionSlug must match [a-z0-9-] (1-100 chars, no leading/trailing dash)",
  ...
  if (!existsSync(questionPath))
    return jsonToolResult(
      { accepted: false, reason: "unknown questionSlug", questionSlug: args.questionSlug },
      true,
    );
  try {
    ...
    return jsonToolResult(
      { accepted: false, reason: errorMessage(error), questionSlug: args.questionSlug },
      true,
    );
  }
}
...
export function precheckContact(
  contact: Contact,
  args: ContactPrecheckInput,
): {
  verdict: "same_day_reply" | "queued" | "decline";
  reason: string;
  matched: string | null;
  advisory_rate: Contact["advisory_rate"];
  ...
  return {
    verdict: "queued",
    reason:
      "No explicit fast path matched; send only if the inquiry is concrete and artifact-tied.",
    matched: null,
    advisory_rate: contact.advisory_rate,
    channels: contact.channels,
  };
...
function verdict(
  kind: "same_day_reply" | "queued" | "decline",
  matched: string,
  contact: Contact,
): ReturnType<typeof precheckContact> {
  return {
    verdict: kind,
    reason: `Matched contact policy: ${matched}`,
    matched,
    advisory_rate: contact.advisory_rate,
    channels: contact.channels,
  };
