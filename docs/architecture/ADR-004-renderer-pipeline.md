...
The renderer now has one more derived surface family: appending `.md` to a canonical public page or content URL returns a static Markdown representation, with `/index.md` representing `/`. These variants are generated by a single catch-all endpoint and a small contract layer in `src/lib/markdown-routes.ts`; selected static pages are maintained summaries, while detail pages preserve the canonical source body. Agent-facing token economics are also derived: `src/lib/agent-metadata.ts` computes summary, word count, approximate `chars/4` token count, Markdown URL, source path, and GitHub source URL for JSON APIs, Markdown envelopes, MCP responses, and the SQLite index. The source model is unchanged: `content/<kind>/*.md` plus per-kind Zod frontmatter schemas remain canonical, while HTML, JSON APIs, `llms.txt`, MCP resources, feeds, and `.md` variants are sibling projections from validated source content and registries.
