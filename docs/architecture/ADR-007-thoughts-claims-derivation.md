...
**Two Anthropic clients** (resolved): `brain-diff-score.ts` and `derive-claims.ts` each instantiated their own client; Slice C's `review-stale-claims` was the third caller, crossing the agreed extraction threshold. The M4.5 cleanup pass extracted `src/lib/anthropic.ts` as the single SDK choke-point — all three callers now compose against `runToolCall`, and the dynamic import inside it keeps every `--no-llm` path SDK-free. Future callers (M5+ background agents) compose against `runToolCall`.
